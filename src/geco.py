...
class GECO(object):
    ...
    def to_cuda(self):
        ...
        self.geco_lambda_min = self.geco_lambda_min.cuda()
        self.geco_lambda_max = self.geco_lambda_max.cuda()
        if self.cma is not None:
            self.cma = self.cma.cuda()

    def to(self, device):
        self.geco_lambda = self.geco_lambda.to(device)
        self.geco_lambda_min = self.geco_lambda_min.to(device)
        self.geco_lambda_max = self.geco_lambda_max.to(device)
        if self.cma is not None:
